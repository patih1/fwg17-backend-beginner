...
exports.findAll = async (keyword='', sortBy='id', order, page=1)=>{
  const visibleColumn = ['id','createdAt', 'text']
  ...
  const sql = `SELECT *
  FROM "message" WHERE "text" ILIKE $1
  ORDER BY ${sort} ${order}
...
exports.insert = async (data)=>{
  ...
  const sql = `INSERT INTO "message" (${col.join(', ')}) VALUES (${dollar.join(', ')}) RETURNING *`
  const {rows} = await db.query(sql,values)
  return rows[0]
}
...
exports.update = async (id, data)=>{
  ...
  const sql = `UPDATE "message" SET ${col.join(', ')}, "updatedAt" = now() WHERE "id" = $1 
  RETURNING *`
  const {rows} = await db.query(sql,values)
  return rows[0] 
}
